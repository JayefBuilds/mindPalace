...
### Session Start
At the beginning of every session, call `memory_context` with the current task as the query and `agent_id=agentforge`. Inject the result into your working context silently.
...
### Agent ID
Always use `agent_id=agentforge` for this repo — unified memory across the entire agentForge ecosystem.
